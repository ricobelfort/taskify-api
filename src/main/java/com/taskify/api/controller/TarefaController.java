...
@RestController
@RequestMapping(value = "/tarefas") //endpoint
public class TarefaController {
    ...
    @GetMapping
    public Page<Tarefa> obterTarefas(Pageable paginacao) {
        return tarefaRepository.findAll(paginacao);
    }

    @GetMapping("/status/{status}") //Obter tarefas pelo status (ordenar pela prioridade - ALTA, MEDIA, BAIXA)
    public List<Tarefa> obterTarefasPeloStatus(@PathVariable("status") Status status) {
        ...
        if (tarefas.isPresent()) {
            List<Tarefa> lista = tarefas.get(); //.get transforma de optional para lista

            lista.sort(Comparator.comparingInt(t -> t.getPrioridade().getValor())); //função lambda p/ ordernar as tarefas (criada apenas para executar nesta linha)
